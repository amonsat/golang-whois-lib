...
func GetWhoisTimeout(domain string, timeout time.Duration) (string, error) {
	...
	for _, server := range servers {
		...
		if IsWhoisDataCorrect(res) {
			CacheWhois.Store(server.zone, server.domain)
...
func GetPossibleWhoisServers(domain string, timeout time.Duration) (whoisServers []*server, err error) {
	...
	if lenDomain > 2 {
		...
		if val, ok := CacheWhois.Load(secondLevelDomain); ok {
			whoisServers = append(whoisServers, &server{val.(string), secondLevelDomain})
		}
	...
	if val, ok := CacheWhois.Load(topLevelDomain); ok {
		whoisServers = append(whoisServers, &server{topLevelDomain, val.(string)})
	}
	...
	if val, ok := CacheIANA.Load(topLevelDomain); ok {
		whoisServers = append(whoisServers, &server{val.(string), topLevelDomain})
		...
		CacheWhois.Store(topLevelDomain, whoisFromIANA)
	}
